    ...
    <div class="container-fluid">
        <div class="card">
            <div class="card-body">
                <div class="row">
                    <div class="col-lg-6">
                        <h4 class="card-title fw-semibold float-start">Publisher</h4>
                    </div>
                    <div class="col-lg-6">
    ...
    <div id="modal-create" class="modal fade" tabindex="-1" aria-labelledby="primary-header-modalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-scrollable modal-md">
            <div class="modal-content">
                ...
                <form action="{{ route('publisher.store') }}" id="form-create">
                    @csrf
                    <div class="modal-body">
                        <div>
                            <label for="publisher" class="form-label">Publisher <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="publisher" name="publisher">
                            <p class="error-message-publisher" style="font-size: 12px;"></p>
                        </div>
                    </div>
...
@section('script')
    <script>
            ...
            const columns = [{
                ...
                },
                {
                    render: function(data, type, full, row) {
                        return `
                    <button onclick="editPublisher(${full.id})"  class="btn bg-warning text-light">Edit</button>
                    <button onclick="deletePublisher(${full.id})" class="btn bg-danger text-light">Delete</button>
                    `
                    }
                },
            ];
        ...
        function editPublisher(id) {
            var baseUrl = "{{ url('/') }}";
            var editUrl = baseUrl + `/publisher/${id}/edit`;
        ...
        function deletePublisher(id) {
            var baseUrl = "{{ url('/') }}";
            var deleteUrl = baseUrl + `/publisher/${id}`;
